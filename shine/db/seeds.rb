...
CustomersBillingAddressCount = CustomersBillingAddress.all.count
CustomersShippingAddressCount = CustomersShippingAddress.all.count

if CustomersBillingAddressCount == 0 || CustomersShippingAddressCount == 0 then

  num_states = State.all.count
  Customer.all.pluck(:id).each do |customer_id|
    create_billing_address(customer_id,num_states) \
      if CustomersBillingAddressCount == 0

    if CustomersShippingAddressCount == 0 then
